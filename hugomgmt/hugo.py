...
def make_diff(name: Path, new_file: Path, old_file: Path | None = None):
    newstat = new_file.stat()
    new_ts = datetime.datetime.fromtimestamp(newstat.st_mtime).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    if old_file:
        oldstat = old_file.stat()
        old_ts = datetime.datetime.fromtimestamp(oldstat.st_mtime).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    else:
    ...
    try:
        ...
        yield from difflib.unified_diff(
            old_lines, new_lines, str(old_name), str(name), old_ts, new_ts
        )
        ...
        new_lines = (
            base64.encodebytes(new_bin).decode("utf-8").splitlines(keepends=True)
        )
        old_lines = (
            base64.encodebytes(old_bin).decode("utf-8").splitlines(keepends=True)
        )
        yield from difflib.unified_diff(
            old_lines, new_lines, str(old_name), str(name2), old_ts, new_ts
        )


@click.option("--theme")
@click.argument(
    "hugodir", type=click.Path(dir_okay=True, exists=True, file_okay=False), default="."
)
def hugo_diff_from_theme(hugodir, theme):
    ...
    for filepath in find_files(paths, ignore_dirs, ignore_files, ["*"]):
        ...
        if themepath.exists():
            # make diff
            _log.info(
                "exists(make diff): filename=%s, relpath=%s, themepath=%s",
                filepath,
                relpath,
                themepath,
            )
            ...
            _log.info(
                "not exists(make all): filename=%s, relpath=%s, themepath=%s",
                filepath,
                relpath,
                themepath,
            )
...
@click.option(
    "--format", type=click.Choice(["yaml", "toml"]), default="toml", show_default=True
)
...
def hugo_yamltoml(format, input, output):
    ...
    if format == "yaml":
        if content:
            print("---", file=output)
        yaml.dump(
            data, stream=output, encoding="utf-8", allow_unicode=True, sort_keys=False
        )
        if content:
            print("---", file=output)
...
def reformat_post(
    basepath: Path, filepath: Path, dry: bool, diff: bool, format: str, format_md: bool
):
    ...
    if format == "yaml":
        if content:
            print("---", file=outfp)
        yaml.dump(
            data, stream=outfp, encoding="utf-8", allow_unicode=True, sort_keys=False
        )
        if content:
            print("---", file=outfp)
