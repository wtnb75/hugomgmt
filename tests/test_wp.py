        ...
        "wp_options": [
            {
                ...
                "option_value": r"/archives/%post_id%",
            },
            {
            ...
            },
            {
                ...
                "option_value": "name123",
            },
            {
                "option_name": "blogdescription",
                "option_value": "descr123",
            },
            {
                "option_name": "siteurl",
                "option_value": "http://example.com/wordpress/",
            },
        ],
        "wp_terms": [
            {"name": "cat1", "slug": "slug1"},
            {"name": "cat2", "slug": "slug2"},
            {"name": "tag1", "slug": "slug3"},
        ],
        "wp_term_taxonomy": [
            {"term_id": 1, "taxonomy": "category"},
            {"term_id": 2, "taxonomy": "category"},
            {"term_id": 3, "taxonomy": "post_tag"},
        ],
        "wp_comments": [
            {"comment_post_ID": 1, "comment_approved": "1"},
...
""",
    ...
    def test_redirect(self):
        ...
        self.assertEqual(0, res.exit_code)
        self.assertIn(
            r"rewrite ^/wordpress/feed/$ /hugopath/index.xml permanent;", res.output
        )
        self.assertIn(
            r"rewrite ^/wordpress/category/slug1(/.*)?$ /hugopath/categories/cat1/ permanent;",
            res.output,
        )
        self.assertIn(
            r"rewrite ^/wordpress/category/slug2(/.*)?$ /hugopath/categories/cat2/ permanent;",
            res.output,
        )
...
"""
        with tempfile.NamedTemporaryFile() as tf:
            ...
            res = CliRunner().invoke(
                self.cli, ["wp-convpost1", "4", "--template", tf.name]
            )
            if res.exception:
                raise res.exception
            self.assertEqual(0, res.exit_code)
    ...
    def test_convpost1_shortcode_resource(self):
        res = CliRunner().invoke(
            self.cli,
            ["wp-convpost1", "4", "--template", "template/post-shortcode.md.j2"],
        )
        if res.exception:
            raise res.exception
        self.assertEqual(0, res.exit_code)
